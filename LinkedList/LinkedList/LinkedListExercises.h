...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // Insert in front if smaller than first node
  else if(newNode->data <= front())
    pushFront(newNode->data);
  // Insert in back if larger than last node
  else if(newNode->data >= back())
    pushBack(newNode->data);
  // Find the node that it is greater than an insert newNode
  else{
    Node *curr = getHeadPtr();
    bool flg = true;
    while(flg){
      if(newNode->data <= curr->data){
        // Place the newNode where the curr node is
        newNode->prev = curr->prev;
        newNode->next = curr;

        // Set the curr->prev to newNode to create head/tail link between newNode and curr
        curr->prev = newNode;

        // Move back to previous node to make new link between previous node's next and newNode
        curr = newNode->prev;
        curr->next = newNode;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // if left is empty, return right.  As right is in sorted order
  if(left.empty()) return right;

  // if right is empty, return left.  As left is in sorted order
  if(right.empty()) return left;

  // To simplify the while loop, end the loop when either left or right is empty
  while(!left.empty() && !right.empty()){
    
    // Determine which number is smallest by comparing the front of each list
  ...
  // If left is not empty, that indicates right was the first to be empty.
  while(!left.empty()){
    merged.pushBack(left.front());
    left.popFront();
  }
  
  // If right is not empty, that indicates left was the first to be empty.
  while(right.size() > 0){
    merged.pushBack(right.front());
    right.popFront();
  }
